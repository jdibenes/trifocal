...
template <typename A>
Eigen::Matrix<typename A::Scalar, Eigen::Dynamic, Eigen::Dynamic> fixed_cost_from_TFT_constraints(Eigen::MatrixBase<A> const& Q)
{
    Eigen::Matrix<typename A::Scalar, Eigen::Dynamic, Eigen::Dynamic> R = Q.householderQr().matrixQR().topRows(27).triangularView<Eigen::Upper>();
